...
class GetPizzaBloc extends Bloc<GetPizzaEvent, GetPizzaState> {
  ...
  GetPizzaBloc(this._pizzaRepo) : super(GetPizzaInitial()) {
    ...
    // on<GetOnePizza>((event, emit) async {
    //   emit(GetOnePizzaLoading());
    //   try {
    //     Pizza pizza = await _pizzaRepo.getPizza(pizzaId);
    //     emit(GetOnePizzaSuccess(pizza));
    //   } catch (e) {
    //     emit(GetOnePizzaFailure());
    //   }
    // });
  }
}
